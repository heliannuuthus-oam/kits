...
function ConvertSelectInner({
	...
	const selectEncoding = async (val: TextEncoding) => {
		const { privateKey, publicKey, pkcsFormat } = getInputs();

		const fromFormat = PkcsFormats[pkcsFormat as PkcsFormat];
		const toFormat = PkcsFormats[pkcsFormat as PkcsFormat];
